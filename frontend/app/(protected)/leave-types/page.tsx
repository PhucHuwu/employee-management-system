...
import { leaveTypeApi } from '@/lib/api/endpoints'
...
export default function LeaveTypesPage() {
  ...
  const fetchItems = useCallback(async () => {
    setIsLoading(true)
    try {
      const data = await leaveTypeApi.getAll({ size: 100 })
      setItems(data.items)
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Không tải được danh sách loại nghỉ phép')
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingItem) {
        await leaveTypeApi.update(editingItem.id, payload)
  ...
  const handleDelete = async () => {
    ...
    try {
      await leaveTypeApi.delete(selectedItem.id)
      toast.success('Đã xóa loại nghỉ phép')
      setDeleteDialogOpen(false)
      await fetchItems()
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Không thể xóa loại nghỉ phép')
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Loại nghỉ phép</h1>
          <p className="text-muted-foreground">Quản lý danh sách loại nghỉ phép trong hệ thống</p>
        </div>
          ...
          {isLoading ? (
            ...
            <Empty>
              <EmptyHeader>
                ...
                <EmptyTitle>Chưa có loại nghỉ phép</EmptyTitle>
                <EmptyDescription>Hệ thống chưa có loại nghỉ phép nào.</EmptyDescription>
              </EmptyHeader>
              <Button onClick={handleOpenCreate} className="mt-4">
                <Plus className="mr-2 size-4" />
                Thêm loại nghỉ phép đầu tiên
              </Button>
            </Empty>
          ) : (
            <Table>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.isPaid ? 'Có' : 'Không'}</TableCell>
                    <TableCell>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Chỉnh sửa loại nghỉ phép' : 'Thêm loại nghỉ phép mới'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="name">Tên loại nghỉ phép</Label>
              <Input
                id="name"
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id="isPaid"
                checked={formIsPaid}
                onCheckedChange={(checked) => setFormIsPaid(checked === true)}
              />
              <Label htmlFor="isPaid">Có lương</Label>
            </div>
